...
# [ENV ADAPTATION] switch Ubuntu apt sources to Tsinghua mirror
# (archive.ubuntu.com / security.ubuntu.com transfers are dropped by the constrained network)
RUN sed -i \
...
# [ENV ADAPTATION] use Tsinghua PyPI mirror (pypi.org is unreachable in this constrained network)
ENV PIP_INDEX_URL=https://pypi.tuna.tsinghua.edu.cn/simple
ENV PIP_TRUSTED_HOST=pypi.tuna.tsinghua.edu.cn
# [ENV ADAPTATION] skip git+ VCS deps (github.com unreachable here; they serve balrog/genesis domains only)
RUN grep -v '^git+' requirements.txt > /tmp/req_nogit.txt && pip install --no-cache-dir -r /tmp/req_nogit.txt

# Install proofgrader for imo_proof domain
# [ENV ADAPTATION] proofgrader_repo is generated by setup_initial.sh (imo domain), not present here; skipped.

# Download things for balrog domains
# [ENV ADAPTATION] balrog post_install needs external network; not a targeted domain; skipped.
